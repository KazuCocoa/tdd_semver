defmodule VersionsTest do
  ...
  describe "compare lhs and rhs" do
    test "1.3.9 is smaller than 1.4.2" do
      {lhs, rhs} = {Versions.semver(1, 3, 9), Versions.semver(1, 4, 2)}
      assert Versions.compare(lhs, rhs) == :rhs
    end

    test "1.3.9 is smaller than 1.3.9" do
      {lhs, rhs} = {Versions.semver(1, 3, 9), Versions.semver(1, 3, 9)}
      assert Versions.compare(lhs, rhs) == :rhs
    end

    test "0.0.2 is bigger than 0.0.1" do
      {lhs, rhs} = {Versions.semver(0, 0, 2), Versions.semver(0, 0, 1)}
  ...
  describe "version up" do
    setup do
      sem = Versions.semver(1, 1, 1)
      {:ok, sem: sem}
    end

    test "major", context, do: assert Versions.up(context.sem, "major") == %Versions{major: 2, minor: 0, patch: 0}
    test "minor", context, do: assert Versions.up(context.sem, "minor") == %Versions{major: 1, minor: 2, patch: 0}
    test "patch", context, do: assert Versions.up(context.sem, "patch") == %Versions{major: 1, minor: 1, patch: 2}
  end
end
